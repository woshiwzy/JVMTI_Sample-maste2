...
extern "C" JNIEXPORT jint JNICALL Agent_OnAttach(JavaVM *vm, char *options,void *reserved) {
    ...
    // 3：事件通知是否可用状态：
//    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_OBJECT_FREE);  // 开启 回收对象监听 OK
//    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_VM_OBJECT_ALLOC); // 开启 jvm 中给对象分配内存 的监听 OK
    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_NATIVE_METHOD_BIND); // 第一次绑定或者调用 native方法  OK

//    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_GARBAGE_COLLECTION_START); // 垃圾回收开始 OK
//    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_GARBAGE_COLLECTION_FINISH); // 垃圾回收结束 OK

//
    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_CLASS_FILE_LOAD_HOOK); // java字节码文件 加载时。OK
//    SetEventNotification(jvmti_env, JVMTI_ENABLE, JVMTI_EVENT_EXCEPTION);  // 开启 java 代码发生异常 的监听  OK
//    SetEventNotification(jvmti_env, JVMTI_ENABLE, JVMTI_EVENT_EXCEPTION_CATCH);// 开启 java 代码发生异常时catch住 的监听  OK
//
...
//    SetEventNotification(jvmti_env, JVMTI_ENABLE,JVMTI_EVENT_FIELD_MODIFICATION); // 字段 修改监听 OK 需要动态绑定
//
...
///*********动态注册 占位方法*************//////////////////
extern "C" JNIEXPORT void JNICALL tempRetransformClasses(JNIEnv *env,jclass clazz,jobjectArray classes) {

}

extern "C" JNIEXPORT jint JNICALL tempCountInstances(JNIEnv *env,jclass clazz,jclass classes) {

}

extern "C" JNIEXPORT void JNICALL tempSetFieldAccessWatch(JNIEnv *env,jclass clazz,jstring classes) {

}

extern "C" JNIEXPORT void JNICALL tempSetFieldModifyWatch(JNIEnv *env,jclass clazz,jstring classes) {

}

extern "C" JNIEXPORT void JNICALL temptestSignalHandler(JNIEnv *env,jclass clazz) {
...
extern "C" JNIEXPORT void JNICALL temp_crawl(JNIEnv *env,jclass clazz, jobject obj) {
    ALOGI("==========temp_crawl=======");
}
extern "C" JNIEXPORT jobjectArray JNICALL temp_getRoots(JNIEnv *env,jclass clazz, jobject obj) {
    ALOGI("==========temp_getRoots=======");
    return NULL;
}
extern "C" JNIEXPORT void JNICALL temp_setTag(JNIEnv *env,jclass clazz, jobject obj) {
    ALOGI("==========temp_setTag=======");
}

extern "C" JNIEXPORT jint JNICALL temp_getObjHashcode(JNIEnv *env,jclass clazz, jobject obj) {
    ALOGI("==========temp_setTag=======");
}

extern "C" JNIEXPORT void JNICALL tempSetBreakPoints(JNIEnv *env,jclass clazz) {
...
extern "C" JNIEXPORT void JNICALL tempregSignalHandler(JNIEnv *env,
    ...
    handler.sa_sigaction = docatch;//异常处理函数
